...
public class Sort<T> {
    ...
    public static Character[] insertionSort(Character[] array, MyMap map, int begin) {
    	...
    	for(int i = 1; i < array.length; i++){
    		insert = binaryInsert(array, array[i], map, 0, i);
    		if(insert == -1) {
    			//Insert at the end
    		}
    		else
    			insertValue(array, insert, i);
    ...
    /**
     * Finds the position to insert the value using binary search
     * @param array The array to search in
     * @param insert The value to place
     * @param map The data to determine the sort order
     * @param begin The value to start sorting
     * @param end The end of the sorted list
     * @return Position to insert the value + 1
     */
    public static int binaryInsert(Character[] array, char insert, MyMap map, int begin, int end) {
    	...
    	if(map.test(array[middle], insert) < 0 && map.test(array[middle + 1], insert) > 0)
    		return middle + 1;
    	
    	if(map.test(array[middle], insert) > 0)
    		return binaryInsert(array, insert, map, begin, middle - 1);
    	else if(map.test(array[middle], insert) < 0)
    		return binaryInsert(array, insert, map, middle + 1, end);
    	else
    		return middle;
    ...
    /**
     * Iterates through the list moving each item over one position
     * Starting from the end of the list and moving until the insert position
     * @param array The array to re-position
     * @param spot The spot to place the value
     * @param old The end the sorted array
     * @return 0 on successful insertion
     */
    public static int insertValue(Character[] array, int spot, int old) {
    	if(spot == old)
    		return 0;
    	
    	Character temp = array[spot];
    	array[spot] = array[old];
    	//Move everything to the right
    	for(int i = old; i > spot; i--) {
    		array[i] = array[i - 1];
    	}
    	//Add the previous position to position + 1
    	array[spot + 1] = temp;
